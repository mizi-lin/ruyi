...
export default function makeManifest(config?: { getCacheInvalidationKey?: () => string; isDev: boolean }): PluginOption {
    const distDir = config?.isDev ? resolve(rootDir, 'dist') : resolve(rootDir, 'build');
